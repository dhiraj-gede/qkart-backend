...
const generateAuthTokens = async (user) => {
  const resp = {
    access: {
      token: generateToken(user._id, Math.floor(Date.now() / 1000) + config.jwt.accessExpirationMinutes * 60, tokenTypes.ACCESS, config.jwt.secret),
      expires: null
    }
  };
  resp.access.expires = new Date(1000 * jwt.decode(resp.access.token).exp)
  return resp;
};
